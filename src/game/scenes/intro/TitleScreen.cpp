...
bool TitleScreen::load(std::string_view data) noexcept
{
    if(!m_isLoaded)
    {
        auto& assets         = m_game->assets;
        auto spaceTexture    = assets.get<sf::Texture>(SPACE_JPG);
        auto duneTexture     = assets.get<sf::Texture>(DUNE_PNG);
        ...
        m_info = std::make_unique<sf::Text>(*font, sf::String::fromUtf8(info.begin(), info.end()));
        ...
        else
        {
            ...
            if(isPresentPlanetTime)
            {
                ...
                if(!is_moving_planet_end)
                {
                    ...
                    const auto floatSize = sf::Vector2f(screenSize);
                    const auto targetPosition = floatSize.x / PLANET_POSITION_FACTOR_X;
...
void TitleScreen::resize(sf::Vector2u size) noexcept
{
    Scene::resize(size);
    const auto screenSize = sf::Vector2f(m_game->window.getSize());
    ...
    {
        {// space and planet
            set_sprite_size(m_space.get(), screenSize);
            set_sprite_size(m_planet.get(), { screenSize.x / PLANET_SCALE_FACTOR_WIDTH, screenSize.y / PLANET_SCALE_FACTOR_HEIGHT });
            m_planet->setPosition({ screenSize.x / PLANET_POSITION_FACTOR_X, screenSize.y / PLANET_POSITION_FACTOR_Y });
        }

        {// buttons (size)
            const float width = screenSize.x / BUTTON_SCALE_FACTOR_WIDTH;
            const float height = screenSize.y / BUTTON_SCALE_FACTOR_HEIGHT;
        ...
        {// buttons (positions)
            const float centerX = screenSize.x / BUTTON_POSITION_FACTOR_X;
            const float centerY = screenSize.y / BUTTON_POSITION_FACTOR_Y;
            const float offset = screenSize.x / BUTTON_SCALE_FACTOR_WIDTH * PLANET_POSITION_FACTOR_X;
    ...
    else
    {
        {// space and planet
            ...
            {
                auto targetPositionX = screenSize.x / PLANET_POSITION_FACTOR_X;
                auto currentPositionX = m_planet->getPosition().x / PLANET_POSITION_FACTOR_X;
                auto difference = currentPositionX - targetPositionX;
                m_planet->setPosition({ targetPositionX + difference, screenSize.y / PLANET_POSITION_FACTOR_Y });
            }
        ...
        {// text info
            const auto center = screenSize * 0.5f;
            auto textBounds = m_info->getLocalBounds();
            m_info->setOrigin(textBounds.size * 0.5f);
            m_info->setPosition(center);

            const float width = screenSize.x / TEXT_INFO_SCALE_FACTOR_WIDTH;
            const float height = screenSize.y / TEXT_INFO_SCALE_FACTOR_HEIGHT;
            set_text_size(m_info.get(), { width, height });
        }
