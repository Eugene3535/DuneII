...
public:
	...
	void addFrame(const std::string& name, const sf::Texture& texture)                                            noexcept;
	void addFrame(const std::string& name, const sf::IntRect& frame)                                              noexcept;

	void createLinearAnimaton(const std::string& name, const sf::Texture& texture, uint32_t duration)             noexcept;
	void createGridAnimaton(const std::string& name, const sf::Texture& texture, uint32_t columns, uint32_t rows) noexcept;
	void createCustomAnimaton(const std::string& name, std::span<const sf::IntRect> frames)                       noexcept;
	
	void loadSpriteSheet(const std::filesystem::path& filePath, const sf::Texture& texture)                       noexcept;
